...
export function getOverlappingGroups(items: Array<{calendarItem?: {start?: {dateTime?: string}, end?: {dateTime?: string}}, durationMinutes?: number}>) {
  const groups: Array<Array<number>> = []
  
  items.filter(item => item.calendarItem?.start?.dateTime).forEach((item, index) => {
    const itemStart = new Date(item.calendarItem?.start?.dateTime!).getTime()
    let foundGroup = false
    
    for (const group of groups) {
      const firstItemInGroup = items[group[0]]
      const groupStartTime = new Date(firstItemInGroup.calendarItem?.start?.dateTime!).getTime()
